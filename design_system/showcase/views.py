...
COMPONENTS = {
    'accordion', 'button', 'text-input', 'textarea', 'select',
    'error-summary', 'panel', 'radios', 'checkboxes', 'banner',
    'item-list', 'summary-list',
}
...
def _nav_context(request, active_section=None, active_slug=None, active_subslug=None):
    ...
    return {
        ...
        'nav_components': [{'slug': c, 'label': c.replace('-', ' ').title()} for c in sorted(COMPONENTS)],
...
def design_system_typography(request, page):
    slug = page.replace('-', '_')
    return render(request, f'design_system/styles/typography/{slug}.html',
                  _nav_context(request, active_section='styles', active_slug='typography', active_subslug=page))


def design_system_style(request, page):
    slug = page.replace('-', '_')
    return render(request, f'design_system/styles/{slug}.html',
...
def item_list_htmx_page(request):
    ...
    return render(request, 'design_system/components/item_list.html', _build_page_context(request))
...
def item_list_htmx_update(request, list_name):
    ...
    if list_name == 'patents':
        ...
        return render(request, 'design_system/components/item_list_htmx_partial.html', {
            'count': count,
            'patent_values': values,
            'htmx_url': reverse('item_list_htmx_update', kwargs={'list_name': list_name}),
        })
...
def design_system_component(request, component):
    ...
    return render(request, f'design_system/components/{slug}.html',
